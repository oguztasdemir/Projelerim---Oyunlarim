...
class Browser:
    ...
    def GetList(self,list): #O tur takip atılacak kullanıcıların adını seçer
        dosya = open("/Users/yarentasdemir/Desktop/Gardrops Bot/Lists/Gardrops_tur.txt","w")
        for i in list:
            dosya.write("{}\n".format(i))
        dosya.close()
        time.sleep(2)
    ...
    def CheckList(self): #Listeyi kontrol eder ve elemanları takip edilecekler listesine yazar
        ...
        global nick
        takip_sayac = 0
        file = open("/Users/yarentasdemir/Desktop/Gardrops Bot/Lists/Gardrops_tur.txt" , "r").readlines() #bu tur seçilen kullanıcılar
        file2 = open("/Users/yarentasdemir/Desktop/Gardrops Bot/Lists/Gardrops_total.txt" , "r").readlines() #tüm kullanıcılar, bunu yapma nedenim bu tur seçilecek kullanıcılar ve total kullanıcılar arasında aynı kişiler varsa onları elemektir
        filecontrol = open("/Users/yarentasdemir/Desktop/Gardrops Bot/Lists/Gardrops_takip.txt" , "w") #takip atılacaklar kısmı
        count = 0
        nick = []
        for line in file:
    ...
    def TakipAt(self): #Takip Atılacaklar Listesindeki kullanıcılara takip atarlar
     try:
        ...
        sayac = 1
        followfile = open("/Users/yarentasdemir/Desktop/Gardrops Bot/Lists/Gardrops_takip.txt" , "r").readlines() #Bu kullanıcılara takip atar
        file3 = open("/Users/yarentasdemir/Desktop/Gardrops Bot/Lists/Gardrops_total.txt","a") #Takip atılan kullanıcıları toplam listesine ekler
        for i in followfile:
            WebDriverWait(Chrome, 0, poll_frequency=0.5, ignored_exceptions=None)
            time.sleep(1)
